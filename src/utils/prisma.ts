...
class PrismaManager {
  ...
  private async getClient(): Promise<{ client: PrismaClient; release: () => void }> {
  ...
  // Middleware auto release connection after timeout
  ...
  public async withTransaction<T>(
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Database transaction timout')), 30000)
        )
      ]);
      return result as T;
    } finally {
...
export { PrismaTransactionClient }